...
\label{sec:intro}
\begin{CJK}{UTF8}{gbsn}
	这是一本使用现代硬件构建语言\myref{https://www.chisel-lang.org/}{Chisel}~\cite{chisel:dac2012}来介绍数字电路系统设计的书。在本书中，我们将关注比常规数字设计书籍更高的抽象层次，使您能够在更短时间内构建更复杂的交互数字系统。
\end{CJK}
...
\begin{CJK}{UTF8}{gbsn}
	这本书的目标读者包含两类开发者：（1）硬件设计工程师 和（2）软件工程师。对于熟悉VHDL或者Verilog，使用诸如Python、Java或Tcl等语言来生成硬件的硬件工程师，可以迁移到包含硬件生成的硬件构建语言。软件工程师也许会对硬件设计感兴趣，未来的Intel芯片将会包含可编程硬件来为程序提速。使用Chisel作为第一种硬件描述语言是一个很好的选择。
\end{CJK}
...
\noindent The functionality of register enable  and initialization at reset can be combined when
